...
Why? I was tired of leaving the house and a train was crossing near my house just where I was trying to go. I wanted to see if I could give myself a notification for whether the train is there. So I set this up to listen for train noises and write to a database if it's detected.
...
It is a quiet corner of the house so the train horn noise comes through clear. I'm only a few blocks away from the train crossing that I want to track. It may be possible to detect the train without putting the mic outside, but initially I was having trouble getting any detections so I wanted as clear a signal as possible.

The audio classifier is here: https://github.com/qiuqiangkong/panns_inference
It takes an audio clip and lists the categories of sounds that it can hear with a confidence score. "Train horn" happens to be a sound category that it supports, so we can simply look for whether the score for "Train horn" is over a certain threshold to decide if a train is passing. 

The classifier pulls from a finite set of categories and simply outputs scores for each category. In other words it outputs the same type of data every time you call it. Because of this, it is very performant and can run on something like a Raspberry Pi. This is in contrast to something like generative AI which is much more performance intensive.

The train near my house also completely stops at the crossing relatively often, which is extra frustrating. So I have also implemented a "train stopping" event looking for another category for "train wheels screeching". If that happens around the time the train horn is detected, you could infer that the train is stopping or at least passing slowly.
